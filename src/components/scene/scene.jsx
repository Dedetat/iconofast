...
const animate = (sel, shapes) => {
  ...
  sel
    ...
    .on('end', () => {
      setTimeout(() => { sel.call(animate, shapes.slice(1).concat([shapes[0]])) }, 200)
    })
}
...
class Scene extends Component {
  attach = idx => (e) => {
    const chromium = [
      `m250.71 2.0762c-100.74-4.482-193.78 66.546-230.53 157.97-23.365 54.92-24.376 118.29-3.154 174.05 34.244 106.31 150.05 172.53 258.99 163.01 62.44-4.37 120.3-37.49 162.12-82.98 84.82-96.43 79.18-261.12-18.71-346.63-46.14-41.081-106.64-65.888-168.72-65.424z M250 250Z`,
  ...
  render() {
    ...
    return (
      <svg style={style} className={className} width={500} height={500}>
        <path ref={this.attach(0)} fill="#9fcaea" />
        <path ref={this.attach(1)} fill="#18173f" />
        <path ref={this.attach(2)} fill="#9fcaea" />
      </svg>
    )
  }
}
